...
export function OrdersPage() {
  ...
  return <div className="page-container orders-page">
    ...
    <div className="orders-list">{orders?.map((order) => <article className="order-card" key={order.id}>
      ...
      <div className="order-products">{order.items.map((item) => <div key={item.id}><img src={imageUrl(item.product?.images?.[0]?.url)} onError={useFallbackImage} alt="" /><span><strong>{item.product?.name ?? "Produto"}</strong><small>{item.quantity} un. · {formatPrice(item.unitPrice)}</small></span></div>)}</div>
